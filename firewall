...
case "$1" in
    start)
        ...
        DOCKERIP=$(echo $DOCKERIP | sed 's/\.\d+\.\d+$/.0.0/')
        DOCKERRANGE="$DOCKERIP/16"
        ...
        for IP in `cat /etc/firewall/ip-whitelist.conf | sed 's/\s/_/g' | grep -v "^#"`; do
            ...
            ## allow each host in our cluster through the forward filter
            $IPTABLES -A FORWARD -s $IP -m state --state NEW -j ACCEPT
        done;
        ...
        cat /etc/firewall/services.conf | grep -v "^#" | while read line; do
            ...
            while IFS=' ' read -ra SERVICES; do
                ...
                ## allow the whitelisted port on the input filter
                $IPTABLES -A INPUT -p $PROTO -s $IP --dport $PORT -m state --state NEW -j ACCEPT

                ## allow the whitelisted port on the forward filter
                $IPTABLES -A FORWARD -p $PROTO -s $IP --dport $PORT -m state --state NEW -j ACCEPT
                
                done <<< "$line"
        done;

        ## allow anything that came from the docker containers
        $IPTABLES -A FORWARD -s $DOCKERRANGE -m state --state NEW -j ACCEPT
        $IPTABLES -A INPUT -s $DOCKERRANGE -m state --state NEW -j ACCEPT

        #$IPTABLES -A FORWARD -s $DOCKERRANGE -d $DOCKERRANGE -m state --state NEW -j ACCEPT

        ## block any other attempt to contact docker containers from the outside
        $IPTABLES -A FORWARD -d $DOCKERRANGE -m state --state NEW -j REJECT
